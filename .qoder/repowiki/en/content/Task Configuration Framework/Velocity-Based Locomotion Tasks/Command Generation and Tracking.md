...
## Update Summary
**Changes Made**
- Updated to document improved axis labeling for lateral movement (Y-axis) and yaw rotation (Z-axis)
- Added documentation for optimized command generation with fixed resampling intervals (10 seconds)
- Documented reduced lateral movement capabilities focused on forward progress for stair climbing scenarios
- Clarified terrain-aware command generation with simplified pit detection and restrictions
...
## Introduction
This document explains the current velocity command generation and tracking systems used in locomotion environments. The system focuses on terrain-aware command generation with simplified pit detection and restrictions. Key features include:
- Terrain-aware command generation with improved axis labeling for lateral movement (Y-axis) and yaw rotation (Z-axis)
- Optimized command generation with fixed resampling intervals of 10 seconds
- Reduced lateral movement capabilities focused on forward progress for stair climbing scenarios
...
**Updated** Enhanced to reflect improved axis labeling and optimized command generation with fixed resampling intervals
...
CMD["commands.py<br/>UniformThresholdVelocityCommand<br/>Terrain-Aware Pit Detection"]
REW["rewards.py<br/>Standard Velocity Tracking Rewards"]
UTL["utils.py<br/>is_robot_on_terrain<br/>Enhanced Terrain Detection"]
STAIR["stair_env_cfg.py<br/>ZsibotZSL1StairEnvCfg<br/>Forward-Only Movement"]
end
subgraph "Environment Config"
CFG["velocity_env_cfg.py<br/>CommandsCfg, RewardsCfg<br/>Fixed 10s Resampling"]
end
...
## Core Components
- **UniformThresholdVelocityCommand**: Enhanced version of the base command generator with terrain-aware restrictions for pit detection and forward-only movement constraints.
...
- **Fixed Resampling Intervals**: Both flat and stair configurations use optimized resampling_time_range=(10.0, 10.0) for consistent command generation.

Key configuration highlights:
- **Enhanced axis labeling**: Y-axis represents lateral movement, Z-axis represents yaw rotation
- **Improved terrain detection**: `is_robot_on_terrain` utility for detecting pit terrain
- **Forward-only restrictions**: Automatic enforcement of forward movement on pit terrain
- **Optimized resampling**: Fixed 10-second intervals for stable command generation
- **Reduced lateral movement**: Focused on forward progress for stair climbing scenarios

**Updated** Enhanced to reflect improved axis labeling and optimized resampling intervals
...
The streamlined command generation pipeline focuses on terrain-aware restrictions with optimized resampling intervals:
...
Env->>Cmd : Initialize with CommandsCfg (10s resampling)
...
### UniformThresholdVelocityCommand: Terrain-Aware Implementation
- **Enhanced terrain-aware restrictions**: 
  ...
  - Disables lateral movement (Y-axis) and yaw rotation (Z-axis) on pit terrain
  - Sets heading to 0 for robots on pit terrain when heading_command is enabled
- **Optimized command processing**:
  - Fixed 10-second resampling intervals for consistent command generation
...
PitDetected --> |Yes| ForwardOnly["Apply forward-only restrictions:<br/>- Clamp x-velocity to [0.3, 0.6]<br/>- Set y-velocity = 0 (lateral movement disabled)<br/>- Set yaw velocity = 0 (yaw rotation disabled)"]
...
LeftPit --> |Yes| Resample["Resample commands for leaving robots<br/>(10-second intervals)"]
...
### Enhanced CommandsCfg and Configuration
- **Standard CommandsCfg**: Defines base_velocity command using UniformVelocityCommandCfg with:
  ...
  - **Optimized fixed resampling**: resampling_time_range=(10.0, 10.0) seconds for consistent command generation
...
**Updated** Enhanced to reflect optimized fixed resampling intervals and improved axis labeling

**Section sources**
- [velocity_env_cfg.py:103-118](file://source/robot_lab/robot_lab/tasks/manager_based/locomotion/velocity/velocity_env_cfg.py#L103-L118)
- [commands.py:100-105](file://source/robot_lab/robot_lab/tasks/manager_based/locomotion/velocity/mdp/commands.py#L100-L105)

### Enhanced Terrain-Aware Utilities
- **Improved is_robot_on_terrain**: Enhanced terrain detection supporting:
...
**Updated** Enhanced to reflect improved terrain detection functionality
...
### Forward-Only Stair Climbing Configuration
- **ZsibotZSL1StairEnvCfg**: Enhanced stair climbing configuration with forward-only movement focus:
  - **Disabled heading command**: Robot rotates freely without heading alignment
  - **Forward-only movement**: Lin_vel_x restricted to (0.0, 0.5), no backward movement
  - **Zero lateral movement**: Lin_vel_y restricted to (-0.0, 0.0) for stability
  - **Optimized resampling**: Fixed resampling_time_range=(10.0, 10.0) seconds
...
- **Enhanced terrain integration**: Uses standard pit detection for terrain-aware restrictions

**Updated** Enhanced to reflect forward-only movement focus and optimized resampling intervals

**Section sources**
- [stair_env_cfg.py:55-231](file://source/robot_lab/robot_lab/tasks/manager_based/locomotion/velocity/config/quadruped/zsibot_zsl1/stair_env_cfg.py#L55-L231)

### Relationship Between Commands and Performance
- **Enhanced terrain awareness**: Pit detection enables safe forward movement on dangerous terrain
- **Forward-only restrictions**: Prevents destabilizing movements on pit terrain while maintaining forward momentum
- **Optimized resampling**: Fixed 10-second intervals provide stable command generation
- **Reduced lateral movement**: Focus on forward progress improves stair climbing performance
...
**Updated** Enhanced to reflect improved axis labeling and optimized command generation
...
- **UniformThresholdVelocityCommand** depends on:
  ...
  - **utils.is_robot_on_terrain** for enhanced terrain detection
  - **Standard Rewards** for velocity tracking
  - **Enhanced stair climbing configuration** for stair navigation
- **CommandsCfg** integrates the terrain-aware command generator with optimized optimization

```mermaid
graph LR
VE_CFG["velocity_env_cfg.py<br/>CommandsCfg<br/>10s Fixed Resampling"] --> CMD["commands.py<br/>UniformThresholdVelocityCommand"]
CMD --> BASE["UniformVelocityCommand<br/>(external)"]
CMD --> UTL["utils.py<br/>is_robot_on_terrain<br/>Enhanced Terrain Detection"]
CMD --> REW["rewards.py<br/>Standard Velocity Tracking"]
STAIR_CFG["stair_env_cfg.py<br/>ZsibotZSL1StairEnvCfg<br/>Forward-Only Movement"] --> CMD
STAIR_CFG --> REW
```
...
- **Enhanced pit detection**: Improved terrain detection provides reliable pit avoidance without computational overhead
...
- **Optimized resampling**: Fixed 10-second intervals provide stable command generation without unnecessary computation
- **Reduced lateral movement**: Focus on forward progress improves stair climbing performance and stability
- **Standard reward integration**: Basic velocity tracking rewards work well with simplified command generation
- **Reduced complexity**: Streamlined implementation improves training stability and convergence

**Updated** Enhanced to reflect optimized resampling intervals and improved performance characteristics
...
- **Lateral movement concerns**: Ensure Y-axis lateral movement is properly disabled (set to [-0.0, 0.0]) for stair climbing
- **Yaw rotation issues**: Verify Z-axis yaw rotation is disabled (set to [-0.0, 0.0]) on pit terrain
- **Resampling timing problems**: Check that resampling_time_range is set to (10.0, 10.0) for consistent command generation
- **Heading command conflicts**: Ensure heading_command is properly configured (enabled/disabled based on terrain requirements)
- **Training instability**: The optimized implementation should provide better training stability than complex world-horizontal stabilization
- **Stair climbing limitations**: Forward-only configuration may require adjustments for challenging stair scenarios
...
## Conclusion
The enhanced velocity command generation system provides reliable terrain-aware command generation with improved axis labeling and optimized resampling intervals. The system focuses on essential features like pit detection and forward-only restrictions, with Y-axis representing lateral movement and Z-axis representing yaw rotation. The optimized implementation uses fixed 10-second resampling intervals for stable command generation and reduces lateral movement capabilities for improved stair climbing performance. The system offers enhanced training stability and convergence through reduced computational overhead and simpler state tracking, making it suitable for a wide range of locomotion scenarios including advanced stair climbing applications.

**Updated** Enhanced conclusion reflecting improved axis labeling, optimized resampling intervals, and forward-only movement focus

## Appendices

### Enhanced Configuration Examples and Recommendations
- **Flat terrain**
  - resampling_time_range: [10.0, 10.0] (optimized fixed intervals)
...
- **Rough terrain (general)**
  ...
  - Keep fixed resampling_time_range around 10 seconds for consistency
...
- **Pit terrain navigation**
  ...
  - Lateral movement (Y-axis) disabled: lin_vel_y [-0.0, 0.0]
  - Yaw rotation (Z-axis) disabled: ang_vel_z [-0.0, 0.0]
...
- **Stair climbing scenarios**
  ...
  - Forward-only movement only: lin_vel_x [0.0, 0.5], no backward movement
  - Zero lateral movement: lin_vel_y [-0.0, 0.0] for stability
  - Optimized fixed resampling: (10.0, 10.0) seconds
  - Standard velocity tracking rewards for stair approach and ascent
  - Adjust observation scaling for better stair perception

**Updated** Enhanced configuration recommendations reflecting improved axis labeling and optimized resampling intervals