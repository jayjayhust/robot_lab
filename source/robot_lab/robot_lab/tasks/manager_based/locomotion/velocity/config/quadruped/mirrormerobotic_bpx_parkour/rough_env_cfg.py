...
"""Manager-based environment configs for MirrorMeroboticBPX parkour locomotion.

This mirrors the ZSL1 parkour environment but uses the BPX robot configuration.
It includes rough/flat/play variants and all ablation study configs.
"""
...
# Pre-defined configs
##
from robot_lab.assets.bpx import MIRRORMEROBOTICS_BPX_CFG  # isort: skip
...
@configclass
class BPXParkourSceneCfg(InteractiveSceneCfg):
    """Scene configuration for BPX parkour with custom terrains."""
    ...
    # robot - MirrorMeroboticBPX with self-collisions
    robot: ArticulationCfg = MIRRORMEROBOTICS_BPX_CFG.replace(
        prim_path="{ENV_REGEX_NS}/Robot",
        spawn=MIRRORMEROBOTICS_BPX_CFG.spawn.replace(
            articulation_props=MIRRORMEROBOTICS_BPX_CFG.spawn.articulation_props.replace(
...
@configclass
class BPXParkourCommandsCfg:
    """Command specifications for BPX parkour (rough terrain: heading mode, fixed 1 m/s)."""
...
@configclass
class BPXParkourActionsCfg:
    """Action specifications for BPX parkour."""
...
@configclass
class BPXParkourObservationsCfg:
    """Observation specifications for BPX parkour (asymmetric actor-critic)."""

    @configclass
    class PolicyCfg(ObsGroup):
        """Actor observations: 52D proprioceptive + optional 187D height scan."""

        # -- proprioceptive (52D total) --
        ...
        foot_contacts = ObsTerm(
            ...
            params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_toe_link"), "threshold": 1.0},
        )  # 4D
        # -- height scan (187D) --
        height_scan = ObsTerm(
...
@configclass
class BPXParkourEventCfg:
    """Domain randomization events for BPX parkour."""
...
@configclass
class BPXParkourRewardsCfg:
    """Reward terms for BPX parkour (flat terrain defaults)."""
    ...
    feet_stumble = RewTerm(
        ...
        weight=-1.0,
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_toe_link"), "ratio": 4.0},
    )
...
@configclass
class BPXParkourTerminationsCfg:
    """Termination terms for BPX parkour."""
...
@configclass
class BPXParkourCurriculumCfg:
    """Curriculum terms for BPX parkour."""
...
@configclass
class BPXParkourRoughEnvCfg(ManagerBasedRLEnvCfg):
    """Manager-based config for BPX parkour rough terrain training."""

    # Scene settings
    scene: BPXParkourSceneCfg = BPXParkourSceneCfg(num_envs=4096, env_spacing=4.0)
    # Basic settings
    observations: BPXParkourObservationsCfg = BPXParkourObservationsCfg()
    actions: BPXParkourActionsCfg = BPXParkourActionsCfg()
    commands: BPXParkourCommandsCfg = BPXParkourCommandsCfg()
    # MDP settings
    rewards: BPXParkourRewardsCfg = BPXParkourRewardsCfg()
    terminations: BPXParkourTerminationsCfg = BPXParkourTerminationsCfg()
    events: BPXParkourEventCfg = BPXParkourEventCfg()
    curriculum: BPXParkourCurriculumCfg = BPXParkourCurriculumCfg()

    base_link_name = "torso"
    foot_link_name = ".*_toe_link"
    # fmt: off
    joint_names = [
...
@configclass
class BPXParkourFlatEnvCfg(BPXParkourRoughEnvCfg):
    """BPX parkour on flat terrain."""
...
@configclass
class BPXParkourRoughPlayEnvCfg(BPXParkourRoughEnvCfg):
    """BPX parkour rough terrain evaluation / play mode."""
...
@configclass
class BPXParkourRoughAbl1EnvCfg(BPXParkourRoughEnvCfg):
    """Ablation 1: remove height scan from policy (actor sees only 52D prop) for BPX parkour."""
...
@configclass
class BPXParkourRoughAbl1PlayEnvCfg(BPXParkourRoughPlayEnvCfg):
    """Ablation 1 play mode for BPX parkour."""
...
@configclass
class BPXParkourRoughAbl2_5EnvCfg(BPXParkourRoughEnvCfg):
    """Ablation 2.5: scan comes first in observation concatenation for BPX parkour.
...
@configclass
class BPXParkourRoughAbl2_5PlayEnvCfg(BPXParkourRoughPlayEnvCfg):
