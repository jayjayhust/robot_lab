...
"""Manager-based environment configs for ZSL1 parkour locomotion.

This mirrors the direct ZSL1 environment (direct/zsl1/) but uses the manager-based
...
@configclass
class ZSL1ParkourSceneCfg(InteractiveSceneCfg):
    """Scene configuration for ZSL1 parkour with custom terrains."""
    ...
    # robot - Unitree ZSL1 with self-collisions
    robot: ArticulationCfg = ZSIBOT_ZSL1_CFG.replace(
        prim_path="{ENV_REGEX_NS}/Robot",
        spawn=ZSIBOT_ZSL1_CFG.spawn.replace(
            articulation_props=ZSIBOT_ZSL1_CFG.spawn.articulation_props.replace(
...
@configclass
class ZSL1ParkourCommandsCfg:
    """Command specifications for ZSL1 parkour (rough terrain: heading mode, fixed 1 m/s)."""
...
@configclass
class ZSL1ParkourActionsCfg:
    """Action specifications for ZSL1 parkour."""
...
@configclass
class ZSL1ParkourObservationsCfg:
    """Observation specifications for ZSL1 parkour (asymmetric actor-critic)."""
    ...
    class PolicyCfg(ObsGroup):
        ...
        foot_contacts = ObsTerm(
            ...
            params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_FOOT_LINK"), "threshold": 1.0},
        )  # 4D
        # -- height scan (187D) --
        height_scan = ObsTerm(
...
@configclass
class ZSL1ParkourEventCfg:
    """Domain randomization events for ZSL1 parkour."""
...
@configclass
class ZSL1ParkourRewardsCfg:
    """Reward terms for ZSL1 parkour (flat terrain defaults)."""
    ...
    feet_stumble = RewTerm(
        ...
        weight=-1.0,
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_FOOT_LINK"), "ratio": 4.0},
    )
...
@configclass
class ZSL1ParkourTerminationsCfg:
    """Termination terms for ZSL1 parkour."""
...
@configclass
class ZSL1ParkourRoughEnvCfg(ManagerBasedRLEnvCfg):
    """Manager-based config for ZSL1 parkour rough terrain training."""

    # Scene settings
    scene: ZSL1ParkourSceneCfg = ZSL1ParkourSceneCfg(num_envs=4096, env_spacing=4.0)
    # Basic settings
    observations: ZSL1ParkourObservationsCfg = ZSL1ParkourObservationsCfg()
    actions: ZSL1ParkourActionsCfg = ZSL1ParkourActionsCfg()
    commands: ZSL1ParkourCommandsCfg = ZSL1ParkourCommandsCfg()
    # MDP settings
    rewards: ZSL1ParkourRewardsCfg = ZSL1ParkourRewardsCfg()
    terminations: ZSL1ParkourTerminationsCfg = ZSL1ParkourTerminationsCfg()
    events: ZSL1ParkourEventCfg = ZSL1ParkourEventCfg()
    curriculum: ZSL1ParkourCurriculumCfg = ZSL1ParkourCurriculumCfg()

    base_link_name = "BASE_LINK"
    foot_link_name = ".*_FOOT_LINK"
    # fmt: off
    joint_names = [  # Go2 is: hip/thigh/calf
    ...
    def __post_init__(self):
        ...
        # -- rough terrain reward overrides (from ZSL1ParkourRoughEnvCfg) --
...
@configclass
class ZSL1ParkourFlatEnvCfg(ZSL1ParkourRoughEnvCfg):
    """ZSL1 parkour on flat terrain."""
...
@configclass
class ZSL1ParkourRoughPlayEnvCfg(ZSL1ParkourRoughEnvCfg):
    """ZSL1 parkour rough terrain evaluation / play mode."""
...
@configclass
class ZSL1ParkourRoughAbl1EnvCfg(ZSL1ParkourRoughEnvCfg):
    """Ablation 1: remove height scan from policy (actor sees only 52D prop) for ZSL1 parkour."""
...
@configclass
class ZSL1ParkourRoughAbl1PlayEnvCfg(ZSL1ParkourRoughPlayEnvCfg):
    """Ablation 1 play mode for ZSL1 parkour."""
...
@configclass
class ZSL1ParkourRoughAbl2_5EnvCfg(ZSL1ParkourRoughEnvCfg):
    """Ablation 2.5: scan comes first in observation concatenation for ZSL1 parkour.
...
@configclass
class ZSL1ParkourRoughAbl2_5PlayEnvCfg(ZSL1ParkourRoughPlayEnvCfg):
